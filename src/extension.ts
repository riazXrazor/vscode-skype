...
import {
  ...
  SkypeUser,
  updateUserList,
  generateSkypeConfigKey
} from "./config/config";
import { SkypeUsersProvider, Dependency } from "./SkypeUsersProvider";
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const skypeUsers = new SkypeUsersProvider();
  vscode.window.registerTreeDataProvider("skypeUsers", skypeUsers);
  ...
  const checkForConfig = async () => {
    try {
      ...
      console.log(
        `${MESSAGE_PREFIX}Error while trying to checkForConfig. ${JSON.stringify(
          _ignorred
        )}`
      );
    }
  ...
  const setSkypeConfig = async () => {
    ...
    const setSkypeConfig = async (newConfig: SkypeConfig) => {
      ...
      vscode.window.showInformationMessage(
        "Skype credentials successfully set."
      );
      return true;
    };
    ...
    const customSetSkypeConfig = async (usernamed?: string) => {
      const username =
        ...
        (await vscode.window.showInputBox({
          ignoreFocusOut: true,
          placeHolder: 'user.username: "riaz@optimizeitsystems.com"',
          prompt:
            "Enter username/phone/email that you use for your skype account."
        }));
      if (!username) {
        vscode.window.showInformationMessage(
      ...
      const password = await vscode.window.showInputBox({
        ignoreFocusOut: true,
        password: true,
        placeHolder: 'user.password: "12345@abc"',
        prompt: "Enter password that you use for your skype account."
      });
      if (!password) {
        vscode.window.showInformationMessage(
    ...
    if (configList.length) {
      const map: Map<string, SkypeConfig> = configList
        .concat(CUSTOM_SKYPE_CONFIG)
        .reduce((map, c) => {
          map.set(generateSkypeConfigKey(c), c);
          return map;
        }, new Map<string, SkypeConfig>());
      const pick = await vscode.window.showQuickPick(Array.from(map.keys()), {
        ignoreFocusOut: true,
        placeHolder: "Select one of previous configs or new custom one."
      });
  ...
  const sendMessage = async () => {
    try {
      const msg = await vscode.window.showInputBox({
        ...
        prompt: "Enter the skype message"
      });
      if (!msg) {
        return;
      }
      let contacts: any = getUserList();
      const pickContacts: any = await vscode.window.showQuickPick(contacts, {
        ignoreFocusOut: true,
        placeHolder: "Select contact to send the message: "
      });
      if (pickContacts) {
        await api.sendMessage({ textContent: msg }, pickContacts.id);
        vscode.window.showInformationMessage(
          `Skype message successfully sent to ${pickContacts.label}`
        );
      }
    } catch (_ignorred) {
      console.log(
        `${MESSAGE_PREFIX}Error while sending message. ${JSON.stringify(
          _ignorred
        )}`
      );
    }
  ...
  const sendMessageTo = async (id: any, label: any) => {
    try {
      const msg = await vscode.window.showInputBox({
        ignoreFocusOut: true,
        placeHolder: "Type your message",
      ...
      if (id && msg) {
        await api.sendMessage({ textContent: msg }, id);
        vscode.window.showInformationMessage(
          `Skype message successfully sent to ${label}`
        );
      }
    } catch (_ignorred) {
      console.log(
  ...
  function createStatusBarItem(): void {
    StatusBarItemNotify = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right,
      100
    );
    StatusBarItemNotify.text = "";

    let toggleTerminalStatusBarItem = vscode.window.createStatusBarItem(
      vscode.StatusBarAlignment.Right,
      100
    );
  ...
  const sendToSkype = vscode.commands.registerTextEditorCommand(
    "vscode-skype.sendToSkype",
    async () => {
  ...
  const getConfigCommand = vscode.commands.registerCommand(
    "vscode-skype.getConfig",
    async () => {
      setSkypeConfig();
    }
  );

  const setConfigCommand = vscode.commands.registerCommand(
    "vscode-skype.setConfig",
    async () => {
      await setSkypeConfig();
    }
  );
  ...
  const sendMessageCommand = vscode.commands.registerCommand(
    "vscode-skype.sendMessage",
    async () => {
      await sendMessage();
    }
  );

  const sendMessageToComand = vscode.commands.registerCommand(
    "vscode-skype.sendMessageTo",
    async (id, label) => {
      await sendMessageTo(id, label);
    }
  );

  const contactList = vscode.commands.registerCommand(
    "vscode-skype.contactList",
    async () => {
      let contacts: any = getUserList();
  ...
  async function getSkypeContacts() {
    ...
    for (const contact of await api.getContacts()) {
      if (!contacts[contact.mri]) {
        contacts.push({
          ...
          icon: contact.profile.avatarUrl
        });
      }
    }
    ...
    for (const contact of await api.getConversations()) {
      if (contact.threadProperties) {
        contacts.push({
          id: contact.id,
          label: contact.threadProperties.topic,
          icon: ""
        });
      }
    }
